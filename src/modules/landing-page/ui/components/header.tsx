...
const Header = ({ isLoggedIn }: { isLoggedIn: boolean }) => {
  ...
  return (
    <motion.header
      ...
      className=" w-[90vw] md:w-[80vw] mx-auto py-2 md:py-4 px-6 md:px-10 rounded-full fixed top-10 inset-x-1/2 -translate-x-1/2 bg-secondary/30  dark:bg-[#0A0A0A]/30 backdrop-blur-lg border z-30"
    >
          ...
          >
            {links.map((link) => (
              <motion.div key={link.name} variants={linkVariants}>
                ...
                >
                  <span className="block  text-foreground/80 hover:text-foreground transition">
                    {link.name}
                  </span>
                </Link>
              </motion.div>
